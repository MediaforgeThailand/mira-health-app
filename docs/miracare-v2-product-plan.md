...
- [❌ 2026-06-15] Cross-platform referral smart link and sales portal completion. Web/app code is complete: SecureStore-backed native attribution, async chat `ref_code` forwarding, auth-time `referral-bind`, `mirahealth://r/<code>` deep links, canonical `/r/<code>` share helpers, real `sales-portal` referrer/order wiring, and retired mock code generation. Evidence: `npm run referrals:store-test`, `npm run referrals:link-audit`, and `npm run v2:verify` green. Live installed-app https handoff and real assisted-order admin-queue proof remain pending owner host/app-link config and signed-in referrer credentials.
